...
def sysisvalid():
    ...
    while True:
        ...
        for service in service_list:
            try:
                Threadlist.append(threading.Thread(target=count_normal,args=(service,)))
                Threadlist[-1].start() 
            except Exception as e:
                print("Server: " + str(e)) 
        while valid_num <= maxnum/2:
            if response_num == maxnum:
                break
        if valid_num > maxnum/2:
            break
...
#     for service in service_list:
#         try:
#             Threadlist.append(threading.Thread(target=heartbeat,args=(service,False,iscommitted,)))
#             Threadlist[-1].start() 
#         except Exception as e:
#             print("Server: " + str(e)) 
    while  append_num <= maxnum/2:
        if react_num == maxnum and append_num <= maxnum/2:
            return False
    return True
...
def updatefile(filename, version, blocklist):
    ...
    if cur_state == 0:
        ...
        #print(log)
        if appendable():#False means append,True means committed
            #print("UpdateFile()")
...
def appendEntries(serverid, _term, _log, _commitIndex):
    ...
    if cur_state != -1:
        ...
        if commitIndex == _commitIndex and len(log) == len(_log):
            ...
            if log[-1] != _log[len(log)-1]:
                i = len(log)-1
                while log[i] != _log[i]:
                    i -= 1
                log = log[:i+1] + _log[i+1:]
                state = 1
            else:
                while len(_log) > len(log):
                    log.append(_log[len(log)])
                    state = 1
            while _commitIndex > commitIndex: 
                commitIndex +=1
                state = 2
                print('updatelocalfile')
                filename = log[commitIndex][1][0]
                version = log[commitIndex][1][1]
                blocklist = log[commitIndex][1][2]
...
def heartbeat(service,isheartbeat,trycommit):
    x = -1
    global cur_state
    cur_tate = 0
    global append_num
    global react_num
    try :
        x = service.surfstore.appendEntries(servernum,term,log, commitIndex)
    except Exception as e:
        pass
    with GlobalMutex:
        if x != 0:
...
def start_leading():
    while cur_state != -1:  
        ...
        for service in service_list:
            try:
                Threadlist.append(threading.Thread(target=heartbeat,args=(service,True,False,)))
                Threadlist[-1].start() 
            except Exception as e:
                print("Server: " + str(e))
        time.sleep(0.1)
...
def Timer():
    ...
    while True:
        ...
        global has_voted
        global timeout
        timeout = random.randint(250,350)
        start = int(time.time() * 1000)
        while cur_state != -1:
